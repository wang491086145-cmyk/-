...
const VELOCITY_FRAGMENT_SHADER = `
  ...
  void main() {
    ...
    // 3. Group Surging Logic
    // Use UV-based noise to create distinct groups of particles
    float groupSeed = fract(sin(dot(vUv, vec2(12.9898, 78.233))) * 43758.5453);
    float groupNoise = snoise(vec3(vUv * 3.0, uTime * 0.15));
    
    // Surging force towards camera (Z direction)
    float surgeIntensity = smoothstep(0.3, 0.7, groupNoise);
    vec3 surge = vec3(0.0, 0.0, 1.0) * surgeIntensity * 0.0025;

    // 4. Local Random Jitter
    // High-frequency noise for "random" local movement
    vec3 jitter = snoiseVec3(pos * 20.0 + uTime * 2.0) * 0.0008;
    ...
    vel.z += flow.z * 0.0006;
    vel += repel + attract + surge + jitter;
...
const POSITION_FRAGMENT_SHADER = `
  ...
  void main() {
    ...
    // Group-based forward flow speed
    float groupID = hash(vUv);
    float baseSpeed = 0.035;
    float groupSpeedBonus = sin(groupID * 10.0 + uTime * 0.5) * 0.015;
    
    pos.z += baseSpeed + groupSpeedBonus; 
...
const PremiumFluidParticles: React.FC = () => {
...
export default PremiumFluidParticles;
